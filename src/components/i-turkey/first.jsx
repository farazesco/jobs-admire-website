// components/InteractiveTableOfContents.jsx (Turkey Version)
...
const InteractiveTableOfContents = () => {
  ...
  const sections = t.turkeyTableOfContents.sections || [];
    ...
    >
      ...
      >
        <button
          ...
          aria-label={t.turkeyTableOfContents.accessibility.toggleMenu}
        >
          {showMobileMenu ? (
            <svg
          ...
          <div className="bg-gradient-to-br from-white via-sky-50 to-white m-4 rounded-2xl overflow-hidden shadow-2xl border border-sky-100 backdrop-blur-sm">
            ...
            <div className="py-8 px-8 bg-gradient-to-r from-sky-500 to-sky-600 relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-sky-100 text-sm mt-2 font-medium">
                  {t.turkeyTableOfContents.subtitle}
                </p>
              </div>
            </div>
            ...
            >
              {sections.map((section) => (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  aria-label={t.turkeyTableOfContents.accessibility.sectionButton.replace(
                    "{title}",
                    section.title
                  )}
                  onKeyDown={(e) => {
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {activeSection === section.id && (
                        ...
                        >
                          ...
                          >
                            {t.turkeyTableOfContents.currentlyReading}
                          </div>
                        </div>
                      )}
                    </div>
            ...
            <div className="py-4 px-6 bg-gradient-to-r from-sky-50 to-white border-t border-sky-100">
              ...
              >
                ...
                ></div>
                {t.turkeyTableOfContents.sectionsCount.replace(
                  "{count}",
                  sections.length
                )}
              </div>
          ...
          {sections.map((section) => (
            ...
            >
              ...
              >
                ...
                <h2
                  className={`text-3xl font-bold bg-gradient-to-r from-sky-700 to-sky-600 bg-clip-text text-transparent ${isRTL ? "mr-12" : "ml-12"}`}
                >
                  {section.title}
                </h2>
              </div>
      ...
      <div className={`fixed bottom-6 z-50 ${isRTL ? "left-6" : "right-6"}`}>
        <div className="relative group">
          <button
            ...
            aria-label={t.turkeyTableOfContents.accessibility.quickNavigation}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-6 w-6"
          ...
          <div
            className={`absolute bottom-full mb-4 w-72 opacity-0 group-hover:opacity-100 transition-all duration-300 transform scale-95 group-hover:scale-100 ${isRTL ? "left-0" : "right-0"}`}
          >
            <div className="bg-white rounded-2xl shadow-2xl p-6 border border-sky-100 backdrop-blur-sm">
              {/* Quick nav header */}
              <div
                className={`flex items-center mb-4 pb-3 border-b border-sky-100 ${isRTL ? "flex-row-reverse" : ""}`}
              >
                <div
                  className={`w-3 h-3 bg-sky-500 rounded-full ${isRTL ? "ml-3" : "mr-3"}`}
                ></div>
                <div className="text-sm font-bold text-sky-700">
                  {t.turkeyTableOfContents.quickNavigation.title}
                </div>
              </div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {sections.map((section) => (
                  <div
                    ...
                    className={`cursor-pointer p-3 rounded-xl transition-all duration-200 flex items-center group/item ${isRTL ? "flex-row-reverse" : ""} ${
                      activeSection === section.id
                    ...
                    role="button"
                    tabIndex={0}
                    aria-label={t.turkeyTableOfContents.accessibility.quickNavSection.replace(
                      "{title}",
                      section.title
                    )}
                    onKeyDown={(e) => {
                  ...
                  >
                    <div
                      className={`w-8 h-8 flex items-center justify-center rounded-full transition-all duration-200 ${isRTL ? "ml-3" : "mr-3"} ${
                        activeSection === section.id
              ...
              <div className="mt-4 pt-3 border-t border-sky-100 text-center">
                <div className="text-xs text-sky-600 font-medium">
                  {t.turkeyTableOfContents.quickNavigation.footer.replace(
                    "{count}",
                    sections.length
                  )}
                </div>
