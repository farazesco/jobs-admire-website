// components/HeroSection.jsx (Turkey Version)
...
const HeroSection = () => {
  ...
  // Get localized destinations
  const destinations = t.turkeyHeroSection.destinations || [];
    ...
    >
      ...
      <div className="container mx-auto h-full px-4 py-8 relative z-20">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 h-5/6 items-center">
          ...
          <div
            className={`text-white z-10 space-y-6 ${isRTL ? "md:pl-8" : "md:pr-8"}`}
          >
            <div
            ...
            >
              ...
              <span className="text-sm font-semibold uppercase tracking-wider text-blue-300">
                {t.turkeyHeroSection.badge}
              </span>
            </div>

            <h1 className="text-5xl md:text-7xl font-bold leading-tight">
              {t.turkeyHeroSection.title}
            </h1>

            <p className="text-xl text-gray-200 max-w-xl">
              {t.turkeyHeroSection.description}
            </p>

            {/* Currently viewing indicator */}
            <div className="pt-6 pb-2">
              <p className="text-sm text-blue-300 uppercase tracking-wide">
                {t.turkeyHeroSection.currentlyViewing.label}
              </p>
            ...
            <div
              className={`flex flex-col sm:flex-row space-y-4 sm:space-y-0 pt-4 ${isRTL ? "sm:space-x-reverse sm:space-x-4" : "sm:space-x-4"}`}
            >
              <button
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-10 rounded-lg transition duration-300 shadow-lg transform hover:-translate-y-1 hover:shadow-xl flex items-center justify-center"
                aria-label={t.turkeyHeroSection.accessibility.exploreButton}
              >
                <span>{t.turkeyHeroSection.buttons.explore}</span>
                <svg
              ...
              <button
                className="bg-transparent hover:bg-white/10 border-2 border-white text-white font-bold py-4 px-10 rounded-lg transition duration-300 backdrop-blur-sm"
                aria-label={t.turkeyHeroSection.accessibility.viewAllButton}
              >
                {t.turkeyHeroSection.buttons.viewAll}
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center z-10 py-8 md:py-0">
            ...
            >
              ...
              {t.turkeyHeroSection.compass.map((direction, i) => {
                const angle = i * 90;
              ...
              {destinations.map((dest, index) => {
                ...
                return (
                  <div
                    ...
                    aria-label={t.turkeyHeroSection.accessibility.destinationImage.replace(
                      "{name}",
                      dest.name
                    )}
                  >
              ...
              <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-150 w-2/5 h-2/5">
                ...
                >
                  ...
                  <img
                    ...
                    alt={t.turkeyHeroSection.accessibility.featuredDestination}
                    className="w-full h-full object-cover"
                  />
                </div>
                ...
                >
                  <div className="text-sm text-blue-300">
                    {t.turkeyHeroSection.destinationCounter
              ...
              <div className="absolute -bottom-16 left-1/2 transform -translate-x-1/2 flex space-x-3">
                {destinations.map((_, index) => (
                  <button
                    ...
                    onClick={() => handleSelectImage(index)}
                    aria-label={t.turkeyHeroSection.accessibility.carouselControl.replace(
                      "{number}",
                      index + 1
                    )}
                  ></button>
