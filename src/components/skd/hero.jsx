...
import {
  BookOpen,
  ArrowRight,
  GraduationCap, // Not used in current render logic
...
const SkillDevelopmentHero = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentWordIndex(
        (prevIndex) =>
          (prevIndex + 1) % t.skilldevelopment.hero.animatedWords.length
      );
    }, 2000);

    return () => clearInterval(interval);
  }, [t.skilldevelopment.hero.animatedWords.length]);
      ...
      <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8 flex items-center">
        <div className="w-full max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start lg:items-center">
                  ...
                  <span className="relative inline-block min-w-[200px] sm:min-w-[280px] md:min-w-[350px] lg:min-w-[420px] xl:min-w-[500px]">
                    ...
                    <span className="absolute top-0 left-0">
                      <span className="font-extrabold text-transparent bg-gradient-to-r from-sky-500 to-blue-600 bg-clip-text animate-pulse">
                        {
                          t.skilldevelopment.hero.animatedWords[
                            currentWordIndex
                          ]
                        }
                      </span>
                  ...
                  <a href="/contact-us" className="block">
                    <button className="group relative w-full py-3 sm:py-4 px-6 font-bold text-white transition-all duration-300 rounded-2xl bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-700 hover:to-blue-700 shadow-lg hover:shadow-xl hover:-translate-y-1 overflow-hidden">
                      ...
                      <div className="relative flex items-center justify-center">
                        ...
                        <span className="mr-2">
                          {t.skilldevelopment.hero.cta.text}
                        </span>
            ...
            {/* RIGHT SIDE - Skill Development Showcase */}
            <div className="space-y-6 order-2 lg:order-2">
              {/* Skill Development Showcase Card (consistent styling) */}
              <div className="relative">
                ...
                <div className="relative p-6 sm:p-8 border shadow-2xl bg-white/90 backdrop-blur-sm border-white/50 rounded-3xl">
                  ...
                  <div className="text-center mb-6 sm:mb-8">
                    <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-2">
                      {t.skilldevelopment.showcase.title}
                    </h2>
                    <p className="text-sm sm:text-base text-gray-600">
                      {t.skilldevelopment.showcase.subtitle}
                    </p>
                  </div>

                  {/* Simple Content */}
                  <div className="space-y-6">
                    {/* Learning Visual (consistent padding, gradients, and border) */}
                    <div className="p-4 sm:p-6 bg-gradient-to-r from-sky-50 to-blue-50 border border-sky-200 rounded-2xl">
                      <div className="flex items-center justify-center">
                        <div className="relative">
                          ...
                          {/* Progress indicators (consistent sizing and animation) */}
                      ...
                      <div className="mt-4 text-center">
                        <div className="text-lg font-semibold text-gray-800 mb-2">
                          {t.skilldevelopment.showcase.learning.title}
                        </div>
                        <div className="text-sm text-gray-600">
                          {t.skilldevelopment.showcase.learning.subtitle}
                        </div>
                      </div>
                    </div>

                    {/* Training Categories (features updated for consistency with ResumeServiceHero) */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      {t.skilldevelopment.showcase.categories.map(
                        (category, index) => {
                          const IconComponent = iconMap[category.icon]; // Dynamically get icon component
                          return (
                            <div
                              key={index}
                            ...
                            >
                              {IconComponent ? (
                                <IconComponent
                                  className={`w-5 h-5 flex-shrink-0 ${category.color || "text-sky-500"}`}
                                />
                              ) : (
                                <div
                                  className={`w-2 h-2 ${category.color || "bg-sky-400"} rounded-full flex-shrink-0`}
                                ></div>
                              )}
                    ...
                    {/* Training Highlights (consistent padding, gradients, border, and font sizes) */}
                    <div className="p-4 sm:p-6 border bg-gradient-to-r from-sky-50 to-blue-50 border-sky-200 rounded-2xl">
                      <div className="grid grid-cols-3 gap-4 text-center">
                        {t.skilldevelopment.showcase.stats.map(
                          (stat, index) => (
                            <div key={index}>
                              <div
...
export default SkillDevelopmentHero;
