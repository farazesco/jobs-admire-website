...
import { 
  ...
  Star,
  TrendingUp, // Not used in current render logic
  Award,
  Sparkles
} from 'lucide-react';
...
const TalentAcquisitionHero = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentWordIndex((prevIndex) => (prevIndex + 1) % t.talentacquisition.hero.animatedWords.length);
    }, 2000);

    return () => clearInterval(interval);
  }, [t.talentacquisition.hero.animatedWords.length]);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start lg:items-center min-h-screen lg:h-full">
            ...
            <div className="space-y-6 lg:space-y-8 order-1 lg:order-1">
              ...
              <div className="inline-flex items-center px-3 sm:px-4 py-2 rounded-full bg-gradient-to-r from-slate-100 to-sky-100 border border-slate-200">
                ...
                <span className="text-sm font-semibold text-sky-600">✨ {t.talentacquisition.hero.badge}</span>
              ...
              <div className="space-y-4">
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold leading-tight">
                  <span className="text-gray-900">{t.talentacquisition.hero.headline.prefix}</span>{" "}
                  <span className="relative inline-block min-w-[200px] sm:min-w-[280px] md:min-w-[350px] lg:min-w-[420px] xl:min-w-[500px]">
                    <span className="opacity-0">{t.talentacquisition.hero.animatedWords[0]}</span>
                  ...
                  <a href="/contact-us" className="block">
                    <button className="group relative w-full py-3 sm:py-4 px-6 font-bold text-white transition-all duration-300 rounded-2xl bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-700 hover:to-blue-700 shadow-lg hover:shadow-xl hover:-translate-y-1 overflow-hidden">
                      ...
                      <div className="relative flex items-center justify-center">
                        ...
                        <span className="mr-2">{t.talentacquisition.hero.cta.text}</span>
            ...
            {/* RIGHT SIDE - Talent Acquisition Showcase */}
            <div className="space-y-6 order-2 lg:order-2">
              
              {/* Talent Acquisition Showcase Card (consistent styling) */}
              <div className="relative">
                ...
                <div className="relative p-6 sm:p-8 border shadow-2xl bg-white/90 backdrop-blur-sm border-white/50 rounded-3xl">
                  ...
                  <div className="text-center mb-6 sm:mb-8">
                    <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-2">
                      {t.talentacquisition.showcase.title}
                    </h2>
                    <p className="text-sm sm:text-base text-gray-600">{t.talentacquisition.showcase.subtitle}</p>
                  </div>
                  
                  {/* Simple Content */}
                  <div className="space-y-6">
                    {/* Talent Search Visual (consistent padding, gradients, and border) */}
                    <div className="p-4 sm:p-6 bg-gradient-to-r from-sky-50 to-blue-50 border border-sky-200 rounded-2xl">
                      <div className="flex items-center justify-center">
                        <div className="relative">
                          ...
                          {/* Search indicators (consistent sizing and animation) */}
                          ...
                          {/* Talent pool indicators (kept for visual consistency, adjust as needed) */}
                      ...
                      <div className="mt-4 text-center">
                        <div className="text-lg font-semibold text-gray-800 mb-2">{t.talentacquisition.showcase.talentPool.title}</div>
                        <div className="text-sm text-gray-600">{t.talentacquisition.showcase.talentPool.subtitle}</div>
                      </div>
                    </div>
                    
                    {/* Recruitment Services (features updated for consistency with other heroes) */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                      {t.talentacquisition.showcase.services.map((service, index) => {
                        const IconComponent = iconMap[service.icon]; // Dynamically get icon component
                        return (
                          <div key={index} className="flex items-center space-x-3 p-4 bg-white/60 rounded-2xl border border-white/50">
                            {IconComponent ? (
                              <IconComponent className={`w-5 h-5 flex-shrink-0 ${service.color || 'text-sky-500'}`} />
                            ) : (
                              <div className={`w-2 h-2 ${service.color || 'bg-sky-400'} rounded-full flex-shrink-0`}></div> 
                            )}
                            <span className="text-xs sm:text-sm font-medium text-gray-700">{service.text}</span>
                          </div>
                        );
                      })}
                    </div>

                    {/* Talent Acquisition Highlights (added for consistency, assuming data structure similar to other heroes' features) */}
                    {/* If you have specific highlight data for talent acquisition, you can replace this with a dedicated section */}
                    {t.talentacquisition.showcase.highlights && ( // Check if highlights data exists
                      <div className="p-4 sm:p-6 border bg-gradient-to-r from-sky-50 to-blue-50 border-sky-200 rounded-2xl">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                          {t.talentacquisition.showcase.highlights.map((highlight, index) => (
...
export default TalentAcquisitionHero;